...
For this study, the researchers created a computational model as a means to detect and quantify the existence of Braess's Paradox within Iloilo City that is based on the DPWH 2024 atlas. It provided a new methodology of evaluating the traffic management system of a complex traffic network in an urban Iloilo City.

The researchers developed a computation model by identifying the primary arterial roads and intersections within Iloilo City. These intersections and roads are then identified as nodes and edges, respectively, which ultimately formed the traffic network system. The volume and direction of travel demand was applied to the network from the OD matrix pairs by zonal areas. Additionally, the annual average daily traffic was then used as weights amongst the edges.

The Iloilo City traffic network was transformed into a computational model under the model framework established by Youn et al. (2008). The Price of Anarchy (PoA) was calculated to determine the critical volume for the identification of Braess's Paradox in the network, while the Bureau of Public Roads (BPR) function was used to update the travel of each edge. The System Optimum baseline (SO) was established to apply systematic delay from the vehicles into the network. 
